...
const Home: NextPage = () => {
  ...
  useEffect(() => {
	...
	const handleForgotPasswordClick = () => {
		alert('To reset your password, change the environment variable for PASSWORD in your SSH or terminal and restart your application.')
	}
  ...
  return (
    <div className='flex flex-col items-center justify-center w-full min-h-screen bg-gray-100'>
      ...
      <div className='mb-6' style={{width: 44}}>
		  {/*<Logo/>*/}
		  LOGO
      </div>
        <h1 className='font-semibold text-2xl mb-4'>Login to your BRAND</h1>
        ...
        <button onClick={handleLoginClick} className='px-8 py-4 text-xl bg-indigo-600 w-full text-white font-semibold rounded-xl hover:bg-indigo-700'>
			Log in
        </button>
      </div>
      <div onClick={handleForgotPasswordClick} className='text-gray-600 mt-12'>
		  Forgot your password?
		  <a className='ml-2 text-indigo-600 font-semibold hover:underline hover:text-indigo-700 cursor-pointer'>Reset password</a>
      </div>
      <div className='text-gray-600 mt-4 text-xs'>Copyright ©{new Date().getFullYear()}</div>
